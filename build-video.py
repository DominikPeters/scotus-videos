...
# concat mp3s
concat_result = os.system(f"ffmpeg -i {argument_mp3} -i mp3/silence.mp3 " + " ".join([f"-i {mp3}" for mp3 in opinion_mp3s]) + f" -filter_complex \"[0:0][1:0]" + "".join([f"[{i+2}:0]" for i in range(len(opinion_mp3s))]) + f" concat=n={len(opinion_mp3s)+2}:v=0:a=1[out]\" -map \"[out]\" mp3/{case_number}-full.mp3")

# build video
# ffmpeg -f concat -i frame-durations.txt -i rucho.mp3 -c:v libx264 -pix_fmt yuv420p -c:a copy -vf fps=20 rucho.mp4
# -t 60 
video_result = os.system(f"ffmpeg -f concat -i frames/{case_number}/frame-durations.txt -i mp3/{case_number}-full.mp3 -c:v libx264 -pix_fmt yuv420p -c:a copy -vf fps=20 videos/{case_number}.mp4")
